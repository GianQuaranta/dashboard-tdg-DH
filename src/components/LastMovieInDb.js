    ...
    class LastMovieInDb extends Component {
        ...
        async componentDidMount() {
            ...
            try {
                ...
                let pedidoProductsDetail = await fetch('http://localhost:3050/api/products/' + id).then(response => response.json())

                console.log('url', `http://localhost:3050/api/products/${id}`);
        ...
        render() {
            ...
            return (
                ...
                style = {
                    { width: 20 + 'rem' } }
                src = { Image }
                alt = " Imagen último producto añadido" / >
                <
                /div>
